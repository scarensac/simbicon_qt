...
//	double decomposeRotation(const Vector3d vB, double *rotAngle, Vector3d* rotAxis) const;
	...
	*/
    ...
    /**
        thse function are just renaming of other functions so that it is easier to read the code...
    */
    inline Vector3d to_world_coordinate(Vector3d vect){return rotate(vect);}
    inline Vector3d to_local_coordinate(Vector3d vect){return getComplexConjugate().rotate(vect);}


    void load_from_matrix(const TransformationMatrix& matrix);

    //this function calculate the delta quaternion from an angular velocity and a time interval
    static Quaternion deltaRotation(const Vector3d& em, double deltaTime)
    {
       Quaternion q;
       Vector3d ha = em * deltaTime *0.5; // vector of half angle
