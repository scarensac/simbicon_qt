...
struct PenalizingEvaluator {
	...
	*/
	...
	void operator()( Function & f, IndividualType& individual )  {
	...
	*/
	...
	void operator()( Function & f, Iterator begin, Iterator end )  {
	...
	/**
	\brief version with multithreading
	*/
	//*
	template<typename Iterator>
	void operator()(SingleObjectiveFunction & f, Iterator begin, Iterator end)  {
		if (f.isThreadSafe()){

			int range = end - begin;
			SHARK_PARALLEL_FOR(int n = 0; n < range; ++n){
...
struct PenalizingEvaluatorWithUserData : public PenalizingEvaluator{
	/**
	* \brief Default c'tor, initializes the penalty factor to \f$10^{-6}\f$.
	*/
	PenalizingEvaluatorWithUserData() : PenalizingEvaluator() {}
	...
	void operator()(Function & f, IndividualType& individual)  {
		...
		if (f.isFeasible(individual.searchPoint())) {
			void * user_data;
			individual.unpenalizedFitness() = f.eval(individual.searchPoint(), individual.id());
			individual.penalizedFitness() = individual.unpenalizedFitness();
		...
		individual.unpenalizedFitness() = f.eval(t, individual.id());
		individual.penalizedFitness() = individual.unpenalizedFitness();
