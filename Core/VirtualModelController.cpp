...
*/
void VirtualModelController::computeTorques(DynamicArray<ContactPoint> *cfs, WaterImpact &resulting_impact){
...
void VirtualModelController::computeJointTorquesEquivalentToForce(Joint* start, const Point3d& pLocal, const Vector3d& fGlobal, Joint* end){
	...
	if (end != NULL){
        tmpV = Vector3d(currentJoint->parent()->getWorldCoordinates(currentJoint->parent_joint_position()), pGlobal);
        torques[currentJoint->idx()] -= tmpV.crossProductWith(fGlobal);
	}
}
