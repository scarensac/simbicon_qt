...
public:
    ...
    TrajectoryComponent(TrajectoryComponent* comp){
        rotationAxis = comp->rotationAxis;
        reverseAngleOnLeftStance = comp->reverseAngleOnLeftStance;
        reverseAngleOnRightStance = comp->reverseAngleOnRightStance;
        bFeedback = NULL;
        offset = comp->offset;
        baseTraj.copy(comp->baseTraj);
    }
    ...
    /**
        ...
        */
    inline Quaternion evaluateTrajectoryComponent(int stance, double phi, const Vector3d& d, const Vector3d& v){
        ...
        double feedbackValue = computeFeedback(d, v);
    ...
    /**
        ...
        */
    inline double computeFeedback(const Vector3d& d, const Vector3d& v){
        if (bFeedback == NULL)
            return 0;
        return bFeedback->getFeedbackContribution(d, v);
    }
    ...
    void computeDorV( double phi, Trajectory1D* trajX, Trajectory1D* trajZ, int stance, Vector3d* result );
    ...
    /**
     * @brief is_implicit
     * @return true is the component is implicit
     */
    bool is_implicit();
...
public:
    ...
    Trajectory(Trajectory* traj){
        leftStanceIndex= traj->leftStanceIndex;
        rightStanceIndex= traj->rightStanceIndex;
        strcpy(jName, traj->jName);
        relative_to_char_frame=traj->relative_to_char_frame;

        //this is the trajectory for the strength of the joing.
        strengthTraj= new Trajectory1D(*traj->strengthTraj);

        //these are the components that define the current trajectory
        for (int i=0;i<traj->components.size();++i){
            TrajectoryComponent* compo= new TrajectoryComponent(components[i]);
            components.push_back(compo);
        }
    }

    void cpy(Trajectory* traj){
        clear();
        ...
        for (int i=0;i<traj->components.size();++i){
            TrajectoryComponent* compo= new TrajectoryComponent(traj->components[i]);
            components.push_back(compo);
        }
    }
    ...
    inline Quaternion evaluateTrajectory(int stance, double phi, const Vector3d& d, const Vector3d& v){
        Quaternion q = Quaternion(1, 0, 0, 0);

        for (uint i=0;i<components.size();i++)
            q = components[i]->evaluateTrajectoryComponent(stance, phi, d, v) * q;
    ...
    void updateComponents(SimBiController* con, Joint* j, Trajectory1D& newDTrajX, Trajectory1D& newDTrajZ, Trajectory1D& newVTrajX, Trajectory1D& newVTrajZ,
                          Trajectory1D* oldDTrajX, Trajectory1D* oldDTrajZ, Trajectory1D* oldVTrajX, Trajectory1D* oldVTrajZ, int nbSamples ) {
        int nbComponents = (int)components.size();
    ...
    /**
     * @brief this function create the components for the implicit controls.
     * meaning if the target is 0 all the time, the user don't have to spécify the component
     * it is set to 0 implicitly. But without this function the component is not created.
     * As such it would be impossible to modify an implicit component during the execution
     */
    void create_implicit_components(const std::vector<Vector3d>& degree_of_freedom_vects);


    /**
     * @brief This function remove the component trajectory for implicit components
     * it is used before saving to a file to not fill file with the implicit components
     */
    void remove_implicit_components();
...
public:
    ...
    /**
                Returns the time we're expecting to spend in this state
        */
    inline void setStateTime(double val){
         stateTime=val;
    }
    ...
    void create_implicit_components(int traj_id, const std::vector<Vector3d>& degree_of_freedom_vects){
        sTraj[traj_id]->create_implicit_components(degree_of_freedom_vects);
    }
