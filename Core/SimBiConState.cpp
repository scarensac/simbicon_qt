...
void TrajectoryComponent::updateComponent(SimBiController* con, Joint* j, Trajectory1D& newDTrajX, Trajectory1D& newDTrajZ, Trajectory1D& newVTrajX, Trajectory1D& newVTrajZ, 
    ...
    for( int i = 0; i < nbSamples; ++i ) {
        ...
        computeDorV( phi, &newDTrajX, &newDTrajZ, LEFT_STANCE, &newD0 );
        computeDorV( phi, &newVTrajX, &newVTrajZ, LEFT_STANCE, &newV0 );
        computeDorV( phi, oldDTrajX, oldDTrajZ, LEFT_STANCE, &d0 );
        computeDorV( phi, oldVTrajX, oldVTrajZ, LEFT_STANCE, &v0 );

        double feedback = computeFeedback(newD0 - d0, newV0 - v0);
...
void TrajectoryComponent::writeRefTrajectory(FILE* f,int speed_idx, int liquid_lvl_idx){
    ...
    fprintf(f, "\t\t\t%s (%lf,%lf)\n", getConLineString(CON_BASE_TRAJECTORY_START),
            ref_trajectories[liquid_lvl_idx].first,ref_traj[speed_idx].first);
...
void TrajectoryComponent::writeTrajectoryComponent(FILE* f){
    ...
    if (ref_trajectories.empty()){
        ...
        fprintf(f, "\t\t\t%s", getConLineString(CON_TRAJ_COMPONENT_LIQUID_LEVELS));
...
bool TrajectoryComponent::is_implicit()
{
    if (!ref_trajectories.empty()){
        return false;
    }

    if (baseTraj.getKnotCount()!=1){
        return false;
    }

    if (baseTraj.getKnotValue(0)!=0.0f||baseTraj.getKnotPosition(0)!=0.5f){
        return false;
    }
...
void TrajectoryComponent::affect_speed_and_liquid_height(double vel, double liquid_level)
{
    if((ref_trajectories.size()>1)&&(ref_trajectories[0].second.size()>1)){
        std::ostringstream oss;
        oss<<"TrajectoryComponent::transform_to_speed_liquid_level_formating: retargetting has been coded only for single 1D traj: (num liquid height//num vel)";
        oss<<"  "<<ref_trajectories.size()<<"   "<<ref_trajectories[0].second.size();
        throw(oss.str());
    }

    if(ref_trajectories.size()==0){
        //copy the base traj to the structure
        ref_trajectories.push_back(RefTrajectoriesMapItem(0,RefTrajectories()));
        ref_trajectories[0].second.push_back(RefTrajectory(0, NULL));
        ref_trajectories[0].second[0].second = new Trajectory1D(baseTraj);
    }


    ref_trajectories[0].first=liquid_level;
    ref_trajectories[0].second[0].first=vel;

}

void TrajectoryComponent::fuse_with(TrajectoryComponent *other)
{
    //if we add a lot of parameters using a recursive function would be needed
    //but the data structure would have to be changed accordingly so I won't care for now
    for (int i=0;i<other->ref_trajectories.size();++i){
        //find the water level an if it does not exists create it
        int id_dim_1=-1;
        for (int j=0;j<ref_trajectories.size();++j){
            if(EQUAL_TO_WITHIN_EPSILON(other->ref_trajectories[i].first,ref_trajectories[j].first)){
                id_dim_1=j;
                break;
            }
        }
        if(id_dim_1<0){
            ref_trajectories.push_back(RefTrajectoriesMapItem(other->ref_trajectories[i].first,RefTrajectories()));
            id_dim_1=ref_trajectories.size()-1;
        }

        //same thing for the desired vel
        for (int j=0;j<other->ref_trajectories[i].second.size();++j){

            //if it already exists send a crash for now
            for (int k=0;k<ref_trajectories[id_dim_1].second.size();++k){
                if(EQUAL_TO_WITHIN_EPSILON(other->ref_trajectories[i].second[j].first,ref_trajectories[id_dim_1].second[k].first)){
                    std::ostringstream oss;
                    oss<<"TrajectoryComponent::fuse_with: can't fuse those tw trajectories they have a compnent in common (liquid_height, veld): ";
                    oss<<ref_trajectories[id_dim_1].first<<"  "<<ref_trajectories[id_dim_1].second[k].first;
                    throw(oss.str());
                }
            }

            //now that we know there is no problem we can simply add the trajectory
            ref_trajectories[id_dim_1].second.push_back(RefTrajectory(other->ref_trajectories[i].second[j].first, NULL));
            ref_trajectories[id_dim_1].second[ref_trajectories[id_dim_1].second.size()-1].second = new Trajectory1D(*(other->ref_trajectories[i].second[j].second));
        }
    }
}
...
void Trajectory::writeTrajectory(FILE* f){
    ...
    //first we need to rmv the implicit components
    std::vector<Vector3d> vect_comp_axis;
    for(int j=0;j<components.size();++j){
        vect_comp_axis.push_back(components[j]->rotationAxis);
    }
    remove_implicit_components();


    //then we cna save the trajectory
    fprintf( f, "\t%s %s\n", getConLineString(CON_TRAJECTORY_START), jName );

    if (relative_to_char_frame)
    ...
    //and we nee to recreate the implicit components so we can use them later
    create_implicit_components(vect_comp_axis);

}


void Trajectory::create_implicit_components(const std::vector<Vector3d> &degree_of_freedom_vects)
{
    for(int i=0; i<degree_of_freedom_vects.size();++i){
        const Vector3d& vect=degree_of_freedom_vects[i];

        //check if the component already exist;
        bool exist=false;
        for(int j=0;j<components.size();++j){
            if (components[j]->rotationAxis==vect){
                exist=true;
                break;
            }
        }
        //if it does go to the next component
        if (exist){
            continue;
        }

        //if it doesn't create it
        TrajectoryComponent* new_comp;
        new_comp=new TrajectoryComponent();
        new_comp->rotationAxis=vect;
        new_comp->reverseAngleOnRightStance=true;
        Trajectory1D traj_1D;
        traj_1D.addKnot(0.5,0);
        new_comp->baseTraj=traj_1D;
        components.push_back(new_comp);
    }
}


void Trajectory::remove_implicit_components()
{
    for(int j=0;j<components.size();++j){
        TrajectoryComponent* cur_comp=components[j];

        if (cur_comp->is_implicit()){
            //now we are sure to have an implicit trajectory...
...
TrajectoryComponent *Trajectory::get_component(Vector3d axis, bool allow_opposite)
{
    for(int j=0;j<components.size();++j){
        TrajectoryComponent* cur_comp=components[j];

        if (cur_comp->rotationAxis==axis){
            return cur_comp;
        }

        if (allow_opposite){
            if (cur_comp->rotationAxis==(axis*(-1))){
                return cur_comp;
            }
        }
...
TrajectoryComponent *Trajectory::get_component(int i){
    if(i<get_component_count()){
        return components[i];
    }
    return NULL;
}
...
void TrajectoryComponent::computeDorV(double phi, Trajectory1D *trajX, Trajectory1D *trajZ, int stance, Vector3d *result) {
    result->y = 0;
