 ...
 * Constructor.
 */
InteractiveWorld::InteractiveWorld(void):Application(){
	bInterface = NULL;
	world = NULL;
}
...
bool InteractiveWorld::onMouseEvent(int eventType, int button, int mouseX, int mouseY){
	...
	if (Globals::drawPushInterface == 1 && button == MOUSE_LBUTTON && eventType == MOUSE_DOWN){
		...
		Point3d p;
		if (bInterface == false){
            std::cerr<<"Warning: No interaction interface was created!"<<std::endl;
			return false;
		}

		if (world == NULL){
            std::cerr<<"Warning: There is no valid reference to a world!"<<std::endl;
			return false;
		}
