...
*/
void CurveEditor::setTrajectory(Trajectory1D* trajectory , Vector3d input_axis) {
...
bool CurveEditor::onMouseEvent(int eventType, int button, int mouseX, int mouseY) {
    ...
    if( pointHeldIdx >= 0 ) {
        ...
        if (keyPressedAlt){
            ...
            double old_val= trajectory->getKnotValue( pointHeldIdx);
            double new_val = pixelYToViewport( editorY );

            double delta=new_val-old_val;

            for (int i=0;i<trajectory->getKnotCount();++i){
                trajectory->setKnotValue( i, trajectory->getKnotValue(i)+delta );
            }
        }else{
