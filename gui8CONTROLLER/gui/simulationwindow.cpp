...
void SimulationWindow::load_from_data()
{
    ...
    ui->check_box_gamepad->setChecked(Globals::gamepad);
...
void SimulationWindow::link_signals()
{
    ...
    connect(ui->check_box_gamepad, SIGNAL(toggled(bool)), this, SLOT(check_box_gamepad_changed(bool)));
...
{
    if (val >= ui->slider_step_width->minimum() &&
        val <= ui->slider_step_width->maximum() ) {
...
{
    if (val >= ui->slider_target_sagittal_speed->minimum() &&
        val <= ui->slider_target_sagittal_speed->maximum() ) {
...
{
    if (val >= ui->slider_target_coronal_speed->minimum() &&
        val <= ui->slider_target_coronal_speed->maximum() ) {
...
{
    if (val >= ui->slider_target_orientation->minimum() &&
        val <= ui->slider_target_orientation->maximum() ) {
...
{
    if (val >= ui->slider_liquid_level->minimum() &&
        val <= ui->slider_liquid_level->maximum() ) {
...
void SimulationWindow::check_box_gamepad_changed(bool val)
{
    Globals::gamepad=val;
}
